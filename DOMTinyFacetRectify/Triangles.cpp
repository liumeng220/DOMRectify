...
#define NO_TIMER //////////////////////////// changed by duanyansong
...
// int OutputMsg(const char *format, ...){return 0;};
// #define OUTPUTMSG OutputMsg
#define OUTPUTMSG cprintf
...
///////////////////////////////////
// By Duanyansong 2003.5.8
void Exit(int code){ OUTPUTMSG("ERROR %d, Abort process.\n", code ); abort(); }
///////////////////////////////////
...
#ifdef _AFXDLL
//	AfxMessageBox( "Build TIN fails. Maybe your input segments intersection each other" );
...
long divconqdelaunay( int *segmentList,int numOfsegment )
{
	...
	if (verbose&&outmsg) { OUTPUTMSG("  Sorting points.\n");	}
	...
	{
		// Add By Duanyansong For Fix Force Use Segment CDT
...
*/
	...
	if (outmsg) OUTPUTMSG( "Total Use Point: %d \n",i );
...
void segmentintersection(struct triedge *splittri,struct edge * splitshelle,point endpoint2)
{
	...
	// Modify by duanyansong for force use segment.
...
int formskeleton(int *segmentList,int *segMarkList,int numOfSegments,char *psegMakList )
{
	...
	if (poly) 
	{
		...
		if (segments > 0) 
		{
			if (verbose&&!quiet) { OUTPUTMSG("  Inserting PSLG segments.\n"); }
			makepointmap();
		}
		...
		if (outmsg) OUTPUTMSG("Total Inserting segments:%d\n",segments );
	...
	if (convex || !poly) 
	{
		...
		if (verbose&& outmsg) {	OUTPUTMSG("  Enclosing convex hull with segments.\n");	}
		markhull();
	}
	return segments;
}
	...
	int numOfPoints,int numberofpointattribs,REAL offx, REAL offy )
{
	...
	if ( outmsg ) OUTPUTMSG("Input xmin,ymin,xmax,ymax = %.3f,%.3f,%.3f,%.3f\n", xmin,ymin,xmax,ymax);
...
struct triangulateio *vorout )
{
	...
	outmsg = triswitches[0]=='Q'?0:1;

	if (outmsg) OUTPUTMSG( "\n------------------CDT START--------------------\n" );
	if ( in->numOfPoints<=3 ){	if (outmsg) OUTPUTMSG("Error:  Input must have at least three input points.\n"); return ; }

	triangleinit();
	parsecommandline( 1, &triswitches );

	// Add By Duanyansong For Fix Bugs ,the float type "=="
	...
	if (outmsg) OUTPUTMSG( "XOFF:%f OFFY:%f\n",offx,offy );
	...
	OUTPUTMSG("checkdelaunay Over\n");

	triangledeinit();
	
	REAL x,y;  if (outmsg) OUTPUTMSG("Input xmin,ymin,xmax,ymax = %.3f,%.3f,%.3f,%.3f\n", xmin+offx,ymin+offy,xmax+offx,ymax+offy);
	...
	if (outmsg) OUTPUTMSG("Output xmin,ymin,xmax,ymax = %.3f,%.3f,%.3f,%.3f\n", xmin,ymin,xmax,ymax);
	if (outmsg) OUTPUTMSG( "\n------------------CDT OVER--------------------\n" );
}
...
void freeTri(triangulateio *tri)
{
	...
	memset( tri,0,sizeof(triangulateio) );
}